...
class UpsertGraphOperation extends InsertGraphOperation {
  ...
  onBefore1(builder) {
    ...
    return this.graph.build(builder);
  }
  ...
  afterGraphInserterCreated(builder, graphInserter) {
    ...
    for (let i = 0, l = insertNodes.length; i < l; ++i) {
      ...
      if (!upsertNode.hasType(UpsertNode.Type.Insert) && !insertNode.handled) {
  ...
  onAfter2(builder, result) {
    // We need to perform operations in a specific order, to respect potential database constraints:
    // delete() needs to occur after unrelate() and update()
    return this.relate(builder)
      .then(() => this.update(builder))
      .then(() => this.unrelate(builder))
      .then(() => this.delete(builder))
      .then(() => super.onAfter2(builder, result));
  }

  unrelateOrDelete(builder, type) {
    const unrelate = type === UpsertNode.Type.Unrelate;
    return Promise.map(
      this.graph.nodes,
      node => {
        ...
        return Promise.map(
          ...
          relName => {
            ...
            const nodes = node.relations[relName].filter(it => it.hasType(type));
  ...
  delete(builder) {
    return this.unrelateOrDelete(builder, UpsertNode.Type.Delete);
  }

  unrelate(builder) {
    return this.unrelateOrDelete(builder, UpsertNode.Type.Unrelate);
  }
